...
export const getServerSideProps: GetServerSideProps = async ({ params: { createOrEdit, id } }) => {
  // @ts-ignore
  if (createOrEdit && ['edit'].includes(createOrEdit)) {
    return {
      props: {
        createOrEdit,
        id,
...
function CreateOrUpdate({ createOrEdit, id }) {
  return (
    <Layout>
      <PageTitle>{createOrEdit} {id && '- ' + id}</PageTitle>
